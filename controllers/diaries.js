...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb)=>{
      console.log(file);
      uniqueName=Date.now()+file.originalname+path.extname(file.originalname)
      cb(null,uniqueName);
  }
});
const upload=  multer({
...
exports.addNoteToDairy = [
  ...
  async (req, res) => {
    ...
    const newNote = {image, email, date, description };
    console.log(newNote);
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        if (findNote) {
          ...
          return res.send({ status: 200, data: "Note added successfully",newNote:newNote });
        }
      } catch (error) {
        return res.send({ status: 500, data: error.message });
      }
